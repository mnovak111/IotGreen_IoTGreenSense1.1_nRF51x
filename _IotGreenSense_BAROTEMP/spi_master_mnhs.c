...
#include "nrf_gpio.h"
#include "common.h"
#include "spi_master_config_mnhs.h" // This file must be in the application folder
...
bool spi_master_tx_rx_ss(uint32_t *spi_base_address, uint8_t ss_pin, uint16_t transfer_size, const uint8_t *tx_data, uint8_t *rx_data)
{
    uint32_t counter = 0;
...
bool spi_master_tx_rx_noss(uint32_t *spi_base_address, uint16_t transfer_size, const uint8_t *tx_data, uint8_t *rx_data)
{
    uint32_t counter = 0;
    uint16_t number_of_txd_bytes = 0;
...
bool spi_master_tx_rx_noss_norxdata(uint32_t *spi_base_address, uint16_t transfer_size, const uint8_t *tx_data)
{
    uint32_t counter = 0;
    uint16_t number_of_txd_bytes = 0;
    uint32_t SEL_SS_PINOUT;
